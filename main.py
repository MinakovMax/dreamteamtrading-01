...
def main():
    ...
    сonvertor = tc.TickConvertor()

    # Use methods that convert tick to candles N seconds
    first = сonvertor.tick_to_candles_N_seconds(file_path=file_path, date_start=date_start, time_start=time_start, date_end=date_end, time_end=time_end, N=1)
    first.to_csv('first.csv', index=False)

    first = сonvertor.tick_to_candles_N_seconds(file_path=file_path, date_start=date_start, time_start=time_start, date_end=date_end, time_end=time_end, N=20)
    first.to_csv('first_20s.csv', index=False)

    # Use methods that convert tick to candles N trade second
    second = сonvertor.tick_to_candles_N_trade_second(file_path=file_path, date_start=date_start, time_start=time_start, date_end=date_end, time_end=time_end, N=1)
    second.to_csv('second.csv', index=False)

    second = сonvertor.tick_to_candles_N_trade_second(file_path=file_path, date_start=date_start, time_start=time_start, date_end=date_end, time_end=time_end, N=20)
    second.to_csv('second_20s.csv', index=False)

    # Use methods that convert tick to candles N change price tick
    third = сonvertor.tick_to_N_change_tick_candles(file_path=file_path, date_start=date_start, time_start=time_start, date_end=date_end, time_end=time_end, N=1)
